...
class ProfileCollector:
    ...
    def _get_metrics_for_server(self, server: str) -> dict:
        ...
        with self._lock:
            ...
            gc_triggered = any(
                t > current_time - self.profile_interval
                for t in gc_events
            )
...
class ServerWithGCUser(HttpUser):
    ...
    @task(3)
    def cpu_heavy(self):
        with self.client.get("/cpu-heavy", catch_response=True, name="[with_gc] /cpu-heavy") as response:
    ...
    def memory_heavy(self):
        with self.client.get("/memory-heavy", catch_response=True, name="[with_gc] /memory-heavy") as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"Status code: {response.status_code}")
    ...
    def network_heavy(self):
        with self.client.get("/network-heavy", catch_response=True, name="[with_gc] /network-heavy") as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"Status code: {response.status_code}")
    ...
    @task(2)
    def io_heavy(self):
        with self.client.get("/io-heavy", catch_response=True, name="[with_gc] /io-heavy") as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"Status code: {response.status_code}")
...
class ServerWithoutGCUser(HttpUser):
    ...
    @task(3)
    def cpu_heavy(self):
        with self.client.get("/cpu-heavy", catch_response=True, name="[without_gc] /cpu-heavy") as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"Status code: {response.status_code}")
    ...
    @task(3)
    def memory_heavy(self):
        with self.client.get("/memory-heavy", catch_response=True, name="[without_gc] /memory-heavy") as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"Status code: {response.status_code}")
    ...
    @task(2)
    def network_heavy(self):
        with self.client.get("/network-heavy", catch_response=True, name="[without_gc] /network-heavy") as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"Status code: {response.status_code}")
    ...
    @task(2)
    def io_heavy(self):
        with self.client.get("/io-heavy", catch_response=True, name="[without_gc] /io-heavy") as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"Status code: {response.status_code}")
    ...
    @task(1)
    def health_check(self):
        with self.client.get("/health", catch_response=True, name="[without_gc] /health") as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"Status code: {response.status_code}")
