...
class GCPredictor(nn.Module):
    ...
    def __init__(
        self, input_size: int = 10, hidden_size: int = 64, num_layers: int = 2, dropout: float = 0.2
    ):
        super().__init__()
...
class GCPredictorWrapper:
    ...
    def add_metrics(self, metrics: dict) -> None:
        ...
        features = np.array([float(metrics.get(k, 0)) for k in feature_order], dtype=np.float32)
...
def train_model(
    ...
    if len(dataset) == 0:
        # Check actual row count in the CSV
        actual_rows = len(dataset.df) if hasattr(dataset, 'df') else 0
        raise ValueError(
            f"Dataset is too small for training. "
            f"Found {actual_rows} rows, but need at least {sequence_length + 1} rows "
            f"(sequence_length={sequence_length} from config.json). "
            f"Run load tests longer to collect more data, or reduce 'sequence_length' in config.json."
        )
    ...
    model = GCPredictor(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers).to(
        device
    )
...
def load_model(model_path: str, device: str = "cpu") -> GCPredictorWrapper:
